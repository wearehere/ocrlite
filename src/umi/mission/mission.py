...
class Mission:
    ...
    def __init__(self):
        ...
        self._msnMutex = threading.Lock()  # 任务队列的互斥锁
        # self.task = None
        self.taskrunning = False
        self._taskMutex = threading.Lock()
        self._schedulingMode = "1111"
        self._threadPool  = concurrent.futures.ThreadPoolExecutor(max_workers= 5)
    ...
    def addMissionWait(self, argd, msnList):
        ...
        nowIndex = 0  # 当前处理的任务
        msnLen = len(msnList)
        condition = threading.Condition()  # 线程同步器
    ...
    #     # 若当前异步任务对象为空，则创建工作线程
    #     self._taskMutex.acquire()  # 上锁
    #     if self._task == None:
    #         self._task = self._Task(self._taskRun)
    #         threadPoolStart(self._threadPool, self._task)
    #     self._taskMutex.release()  # 解锁

    def _startMsns(self):
        self._taskMutex.acquire()
        if not self.taskrunning:
            self._threadPool.submit(self._taskRun)
            self.taskrunning = True
        self._taskMutex.release()
    ...
    def _taskRun(self):  # 异步执行任务字典的流程
        ...
        while True:
            ...
            t1 = time.time()
            logging.info("[mission._taskRun] start msnTask")
            try:
                res = self.msnTask(msnInfo, msn)
            except Exception as e:
                # res = {"error": f"[Error] {e}"}
                logging.debug( f"[msnTask Error] {e}")
            t2 = time.time()
    ...
    def _taskFinish(self):  # 任务结束
        logging.debug("[mission._taskFinish]")
        self._taskMutex.acquire()  # 上锁
        # self._task = None
        self.taskrunning = False
        self._taskMutex.release()  # 解锁
