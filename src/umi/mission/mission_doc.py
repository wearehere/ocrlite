...
class _MissionDocClass(Mission):
    ...
    def addMission(self, msnInfo, msnPath, pageRange=None, pageList=[], password=""):
        ...
        doc = FitzFile(msnPath, password = password)
        if(not doc.open()):
           return f"[Error] fitz.open error: {msnPath} "
        docmeta = doc.get_metainfo()
        if(not docmeta["authenticate"]):
            doc.close()
            return "[Error] 文档已加密，请提供正确密码。"
        ...
        if len(pageList) == 0:
            if isinstance(pageRange, (tuple, list)) and len(pageRange) == 2:
                ...
                if b > page_count:
                    return f"[Error] pageRange {pageRange} 范围结束不能大于页数 {page_count}"
                if a > b:
    ...
    def msnTask(self, msnInfo, pno):  # 执行msn。pno为当前页数
        logging.debug("[mission_doc]msnTask entry")
        ...
        logging.debug(f"[mission_doc]extractionMode {extractionMode}")
        # =============== 提取图片和原文本 ===============
        imgs = []  # 待OCR的图片列表
        tbs = []  # text box 文本块列表
        if extractionMode == "fullPage":  # 模式：整页强制OCR
            ...
            imgs.append(doc.get_img_fullpage(pno))
        else:
            textmode = extractionMode == "textOnly" or extractionMode == "mixed"
            imgmmode = extractionMode == "imageOnly" or extractionMode == "mixed"
            blocks = doc.get_blocks(pno,textmode, imgmmode)
            if imgmmode:
                imgs = blocks["imgs"]
            if textmode:
                tbs = blocks["txts"]
        ...
        logging.debug(f"[mission_doc]imgs {len(imgs)}")
        if imgs:
            ...
            for o in ocrList:
                res = o["result"]
                logging.debug(f"[mission_doc]ocr results {res}")
                if res["code"] == 100:
                    x, y = o["xy"]
                    scale = o["scale"]
                    for r in res["data"]:
                        ...
                        r["from"] = "ocr"  # 来源：OCR
                        tbs.append(r)
                        logging.debug(f"[mission_doc]***{r}")
                elif res["code"] != 101:
                    errMsg += f'[Error] OCR code:{res["code"]} msg:{res["data"]}\n'

        # =============== tbpu文本块后处理 ===============
        # 忽略区域
        logging.debug(f"[mission_doc]imgs tbpu文本块后处理")
        ...
        # =============== 组装结果字典 resDict ===============
        logging.debug(f"[mission_doc]组装结果字典")
        if errMsg:
