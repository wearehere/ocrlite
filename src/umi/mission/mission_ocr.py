...
class __MissionOcrClass(Mission):
    ...
    def addMissionList(self, msnInfo:map, msnList:map):  # 添加任务列表
        # 实例化 tbpu 文本后处理模块
        msnInfo["tbpu"] = []
        argd = msnInfo["argd"]
        # 忽略区域
    ...
    def msnTask(self, msnInfo, msn):  # 执行msn
        if not "bytes" in msn:
            return {
                "code": 901,
                "data": f"[Error] support bytes task type only"
            }
        logging.debug(f"[mission_ocr.msnTask]:msnInfo {msnInfo}")
        ocrtets, elapse = self._api(msn["bytes"])

        ## use rapidocr output format
        logging.debug(f"[mission_ocr.msnTask]:len result {len(ocrtets)}")
        tbs = []
        score = 0
        for ocrret in ocrtets:
            tb = {
                "text": ocrret[1],
                "score": ocrret[2],
                "box": ocrret[0]
            ...
            score += ocrret[2]
        res = {}
        res["score"] = score/len(ocrtets)
        res["data"] = tbs
        logging.debug(f"[mission_ocr.msnTask]:score {res['score']}")

        if msnInfo["tbpu"]:
            for tbpu in msnInfo["tbpu"]:
                res["data"] = tbpu.run(res["data"])
                logging.debug(f"[mission_ocr.msnTask]res data {res['data']}")

        res["code"] = 100  # todo, return error code if ocr fail
        return res
