...
class BatchDOC():
    ...
    # 添加一些文档
    @staticmethod
    def addDocs(paths, isRecurrence = False):
        paths = utils.findDocs(paths, isRecurrence)
        docs = []
        for p in paths:
            # info = MissionDOC.getDocInfo(p)
            # if "error" in info:
            mtinfo = FitzFile.get_doc_metainfo(p)
            if not mtinfo:
                logging.info(f'[Warning] 读入文档失败：{p}')
                continue
            docs.append({"path":p, "page_count":mtinfo["pagecount"]})
        # 返回：{ "path" , "page_count" }
        return docs
    ...
    def msnDocs(self, docs:list, argd:map):
        if self._msnIdPath:
        ...
        for d in docs:
            ...
            if type(output) == str:  # 创建输出器失败
                ...
                continue
            # 任务信息
            logging.debug(f"[BatchDOC.msnDocs]output {output}")
            msnInfo = {
            ...
            logging.debug(f"add Mission return {msnID}")
    ...
    # 初始化输出器列表。成功返回两个输出器列FitzFilemission.dir"])  # 转绝对地址
    def _initOutputList(self, argd, path):
        ...
        logging.debug(f"[BatchODC.__initOutputList] {outputArgd}")
        ...
        logging.debug(f"[BatchODC.__initOutputList]output {output}")
        return output

    # ========================= 【任务控制器的异步回调】 =========================

    def _onStart(self, msnInfo):  # 一个文档 开始
        logging.debug("[BatchDOC._onStart]")
        msnID = msnInfo["msnID"]
        if msnID not in self._msnIdPath:
            logging.warning(f"[Warning] _onStart 任务ID未在记录。{msnID}")
            return
        logging.debug(f"_onStart {msnInfo['path']}")
        # self.callQmlInMain("onDocStart", msnInfo["path"])

    def _onReady(self, msnInfo, page):  # 一个文档的一页 准备开始
        page += 1
        logging.debug(f"BatchDOC._onReady {msnInfo['path']}")

    def _onGet(self, msnInfo, page, res):  # 一个文档的一页 获取结果
        logging.debug("[BatchDOC._onGet]")
        page += 1
        msnID = msnInfo["msnID"]
        if msnID not in self._msnIdPath:
            logging.info(f"[Warning] _onGet 任务ID未在记录。{msnID}")
            return
        ...
        for o in output:  # 输出
            try:
                ...
                logging.info(f"文档结果输出失败：{o}\n{e}")
        logging.debug(f"_onGet {msnInfo['path']} {page} {res}")

        # self.callQmlInMain("onDocGet", msnInfo["path"], page, res)

    def _onEnd(self, msnInfo, msg):  # 一个文档处理完毕
        # msg: [Success] [Warning] [Error]
        logging.debug("[BatchDOC._onEnd]")
        msnID = msnInfo["msnID"]
        if msnID not in self._msnIdPath:
            logging.info(f"[Warning] BatchDOC._onEnd 任务ID未在记录。{msnID}")
            return
        ...
        logging.debug(f"BatchDOC._onEnd {msnInfo['path']} {msg}")
        if not self._msnIdPath:  # 全部完成
            msg = "[Success] All completed."
            logging.info(f"BatchDOC._onEnd {msnInfo['path']} {msg}")
        # self.callQmlInMain("onDocEnd", msnInfo["path"], msg)

    def taskRemain(self):
        return len(self._msnIdPath)
